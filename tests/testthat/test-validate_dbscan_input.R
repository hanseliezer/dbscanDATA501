...
blobs_str <- blobs
blobs_str$X20 <- c(rep("A", 200), rep("B", 300))
# adding column of booleans
blobs_logi <- blobs
blobs_logi$X21 <- c(rep(TRUE, 200), rep(FALSE, 100), rep(TRUE, 200))
# has NAs
blobs_NA <- blobs
blobs_NA[66, 2] <- NA

# actual distance matrix
blobs_distmat <- as.matrix(dist(blobs))
...
test_that("Verify data argument", {
  ...
  expect_error(dbscan(blobs['X1' < -1000], test_eps, test_min_pts),
               "Please supply a dataset/distance matrix")
  ...
  # data can't have any NAs at all
  expect_error(dbscan(blobs_NA, test_eps, test_min_pts),
               "data must not contain any 'NA's/'NaN's.")
  
  # data types in dataframe must be all numeric
  expect_error(dbscan(blobs_str, test_eps, test_min_pts),
               "data must consist entirely of numeric type.")
  
  # data types in dataframe must be all numeric
  expect_error(dbscan(blobs_logi, test_eps, test_min_pts),
               "data must consist entirely of numeric type.")
  
  # data is of correct type, columns are of correct type
  expect_silent(dbscan(blobs, test_eps, test_min_pts))
...
test_that("Verify min_pts and eps arguments", {
  ...
  # can't be strings
  expect_error(dbscan(blobs, "9", test_min_pts),
               "eps must be integers equal or bigger than 0.")
  expect_error(dbscan(blobs, test_eps, "8"),
               "min_pts must be integers equal or bigger than 0.")
  
  # can't be logical
  expect_error(dbscan(blobs, FALSE, test_min_pts),
               "eps must be integers equal or bigger than 0.")
  expect_error(dbscan(blobs, test_eps, TRUE),
               "min_pts must be integers equal or bigger than 0.")
  
  # can't be dataframes
  expect_error(dbscan(blobs, blobs, test_min_pts),
               "eps must be integers equal or bigger than 0.")
  expect_error(dbscan(blobs, test_eps, blobs_str),
               "min_pts must be integers equal or bigger than 0.")
  
  # eps is checked first before min_pts. so if both params are wrong, the message
  # will be related to eps
  expect_error(dbscan(blobs, "888", TRUE),
               "eps must be integers equal or bigger than 0.")
...
test_that("Verify metric argument", {
  ...
  expect_error(dbscan(blobs, test_eps, test_min_pts, metric='chebyshev'),
               "Options for distance metric are 'euclidean', 'manhattan' or 'precomputed'.")
  
  # available metric
  expect_silent(dbscan(blobs, test_eps, test_min_pts, metric='euclidean'))
  
  # available metric
  expect_silent(dbscan(blobs, test_eps, test_min_pts, metric='manhattan'))
...
test_that("Verify shape of data/distance matrix", {
  ...
  # distance matrix
  expect_error(dbscan(blobs, test_eps, test_min_pts, metric='precomputed'),
...
test_that("Verify normalise and border_pts arguments", {
  ...
  expect_error(dbscan(blobs, test_eps, test_min_pts, normalise=9),
               "normalise must be TRUE/FALSE.")
  
  # border_pts must be logical
  expect_error(dbscan(blobs, test_eps, test_min_pts, border_pts="peep"),
  ...
  expect_error(dbscan(blobs, test_eps, test_min_pts, border_pts="peep", normalise=55),
               "normalise must be TRUE/FALSE.")

  # correctly set to logical
  expect_silent(dbscan(blobs, test_eps, test_min_pts, normalise=FALSE, border_pts=FALSE))
